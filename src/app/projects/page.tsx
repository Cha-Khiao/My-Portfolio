...
import { ArrowLeft, Search, X, Layers, ChevronDown } from 'lucide-react';
import { ProjectCard } from '@/components/ProjectCard';
import { ProjectData, defaultProjects } from '@/lib/initial-data';
...
export default function AllProjectsPage() {
  ...
  React.useEffect(() => {
    fetch('/api/projects')
      ...
      .catch(() => console.warn('Using local fallback projects'));
  }, []);
    ...
    <div className="max-w-[920px] mx-auto px-6 pt-24 sm:pt-28 md:pt-32 pb-16 md:pb-24">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-fg-secondary text-xs sm:text-sm mt-1.5">
              ผลงานและโปรเจกต์ดิจิทัลทั้งหมด พร้อม Live Demo Preview และซอร์สโค้ด
            </p>
          </div>
      ...
      <div className="mb-8">
        <div className="relative w-full">
          ...
          <input
            ...
            placeholder="ค้นหาชื่อโปรเจกต์ คำอธิบาย หรือเทคโนโลยี..."
      ...
      {visibleProjects.length === 0 ? (
        <div className="p-12 text-center rounded-2xl bg-card border border-border space-y-3">
          ...
          <h3 className="font-outfit text-base font-bold text-foreground">ไม่พบผลงานที่ตรงกับคำค้นหา</h3>
          <p className="text-xs text-fg-secondary">ลองเปลี่ยนคำค้นหาใหม่อีกครั้ง</p>
          <button
            onClick={() => setSearchQuery('')}
      ...
      {hasMore && (
        <div className="text-center mt-10">
          ...
          >
            แสดงผลงานเพิ่มเติม ({filteredProjects.length - visibleCount} รายการ)
